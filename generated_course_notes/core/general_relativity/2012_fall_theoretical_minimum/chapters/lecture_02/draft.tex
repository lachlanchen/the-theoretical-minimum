...
\section{Triangulations and the Meaning of Curvature}
...
This gives the lecture a definite goal. To decide whether a space is really flat, one is typically given the metric tensor. But before we can use the metric, we need the language of tensors.
...
V = V^m E_m.
\end{equation}
The coefficients \(V^m\) are numbers, not vectors. They tell us how much of each basis vector is needed to build \(V\). These are the contravariant components.
...
\begin{equation}
V \cdot V = (V^m E_m)\cdot (V^n E_n) = g_{mn}V^mV^n.
\end{equation}
...
The lecture then refines the point. Orthogonality by itself is not enough to recover Cartesian simplicity. Polar coordinates are orthogonal, but the separation corresponding to a fixed angular change grows with radius. So even when coordinate directions meet at right angles, the basis vectors need not behave as they do in Cartesian coordinates.

\section{Tensors by Their Transformation Laws}

Having inserted the geometric picture, the lecture returns to the official viewpoint: at first, tensors are defined not by geometric intuition but by how their components transform when we change coordinates.

Let \(x^m\) and \(y^m\) be two coordinate systems on the same space, related by an invertible differentiable change of variables. The prime notation will refer to components written in the \(y\)-coordinates, and the unprimed notation to components written in the \(x\)-coordinates.
...
\end{align}
The differential \(dx^m\) is the model example. It is a contravariant vector because its components transform with the Jacobian \(\partial y/\partial x\).

A covariant vector transforms the other way:
\begin{equation}
W'_m = \frac{\partial x^n}{\partial y^m}W_n.
\end{equation}
The gradient of a scalar is the model example:
\begin{equation}
\frac{\partial S}{\partial y^m}
=
\frac{\partial x^n}{\partial y^m}\frac{\partial S}{\partial x^n}.
\end{equation}
This is just the chain rule, written in tensor notation.

The general rule is systematic. Each upper \(y\)-index contributes a factor of \(\partial y/\partial x\), and each lower \(y\)-index contributes a factor of \(\partial x/\partial y\). Thus a mixed rank-two tensor transforms as
\begin{equation}
T'^m{}_n
=
...
This is where Susskind's opening remark about notation pays off. The indices do not merely label components. They tell us what kind of object we are manipulating, and they tell us what sort of Jacobian factors must appear in any valid transformation law.

\section{Tensor Equations, Tensor Products, and Contraction}

Once tensors are defined by their transformation laws, we immediately get their most important use: tensor equations have the same truth value in every coordinate system, provided the two sides have the same index structure.
...
\begin{remark}
A student later phrases this sharply: the argument depends on the linearity of the transformation laws in the tensor components. That is exactly right.
\end{remark}

This is why one may move every term of a tensor equation to one side and set the result equal to zero. The lecture makes this concrete for subtraction. If \(T\) and \(S\) are tensors of the same type, then
\begin{equation}
T^{m\cdots}{}_{\cdots p}-S^{m\cdots}{}_{\cdots p}
=
(T-S)^{m\cdots}{}_{\cdots p}.
\end{equation}
The ellipses are deliberate: the board suppresses the full rank, and we should not invent it. What matters is that the index pattern matches on both sides.
...
T^m{}_n = V^m W_n
\end{equation}
is a rank-two tensor. This is the tensor product, or outer product. It is \emph{not} the dot product. In four dimensions it has sixteen components, not one.
...
This is the lecture's tensor-algebra version of the inner product: contraction lowers rank by two and preserves tensor character when it is done correctly.
...
The lecture announces one more tensor operation, covariant differentiation, but does not develop it yet. The real mathematical payoff of the present lecture comes next, when we return to the metric tensor.

\section{The Metric Tensor as the Quadratic Form of Distance}

Now we come back to the object that was promised near the beginning. Consider a little displacement vector with contravariant components \(dx^m\). Its squared length is defined by the metric tensor:
\begin{equation}
...
Several points are already built into this formula. First, the metric can depend on position. Second, the expression is quadratic in the coordinate differentials. Third, the symmetry of \(dx^m dx^n\) means that the antisymmetric part of \(g_{mn}\) plays no role, so for the present discussion we may take the metric to be symmetric.

The lecture then asks how many independent components the metric has. In four dimensions there are initially \(4\times 4=16\) coefficients, but symmetry reduces this to \(10\). In three dimensions the count is \(6\), and in two dimensions it is \(3\):
\begin{equation}
...
This also explains why the metric carries two lower indices: it multiplies two contravariant displacement components \(dx^m\) and \(dx^n\).
...
g_{mn}=g_{nm}.
\end{equation}
In other words, the metric is a symmetric matrix.

For the ordinary Riemannian geometry being discussed in this lecture, Susskind then assumes that the metric has no zero eigenvalues. The matrix is therefore invertible. We denote its inverse by \(g^{mn}\), and define it through
\begin{equation}
g_{mn}g^{np}=\delta_m{}^p.
\end{equation}
This is the tensor version of the statement that a matrix multiplied by its inverse gives the identity matrix.

A late student question asks what space these tensors live in. The useful answer is the tangent space. At each point of the curved space there is a tangent space of the same dimension, and vectors and higher tensors at that point are naturally regarded as living there. This is why the index ranges match the dimension of the underlying space.
...
\end{equation}
or the opposite sign convention. So the later Lorentzian metric will not be positive definite. Even so, the matrix can remain invertible; null vectors do not imply zero eigenvalues.
...
The remaining step, deferred to the next lecture, is differentiation. If the question of flatness is to be answered in earnest, we must know how the metric varies from point to point. That means covariant differentiation, and then curvature.